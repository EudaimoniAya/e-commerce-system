...
- MockLLMClient 返回编程输出（str / JSON dict），不访问网络
...
from app.ai.llm.client import DeepSeekClient, MockLLMClient


@allure.epic("ai")
@allure.feature("llm_client")
@allure.title("MockLLMClient 返回编程 JSON 输出。")
@pytest.mark.asyncio
async def test_mock_llm_returns_programmed_json() -> None:
    """dict 输出序列化为 JSON 字符串（供 NL 网关解析）。"""
    client = MockLLMClient(output={"intent": "knowledge", "confidence": 0.9})
...
@allure.epic("ai")
@allure.feature("llm_client")
@allure.title("MockLLMClient 原样返回 str 输出。")
@pytest.mark.asyncio
async def test_mock_llm_returns_plain_string() -> None:
    """str 输出原样返回（生成答案场景）。"""
    client = MockLLMClient(output="你好，这是答案")
