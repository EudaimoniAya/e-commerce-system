...
class TestCreateApp:
    """create_app 工厂（spec: Logging setup invoked from create_app）。"""
    ...
    def test_create_app_has_project_title(self) -> None:
        """创建的应用带有项目标题。"""
        from app.main import create_app

        app = create_app()
        assert app.title == "e-commerce-system"

    def test_create_app_includes_expected_routers(self) -> None:
        """create_app 挂载了健康探针和业务域路由。"""
        from app.main import create_app

        app = create_app()
        route_paths = {route.path for route in app.routes}

        assert "/health" in route_paths, "应挂载 health 路由"
        assert "/auth/register" in route_paths, "应挂载 user 路由"
        # catalog/ordering 路由至少存在其一
        has_shops = "/shops" in route_paths or any(
            p.startswith("/shops") for p in route_paths
        )
        assert has_shops, "应挂载 catalog 路由"


class TestAppEnvInTest:
    """测试环境 APP_ENV=test（spec: Test environment APP_ENV）。"""

    def test_task_test_uses_test_app_env(self) -> None:
        """task test / CI 运行时 Settings.app_env 为 test。"""
        from app.infra.config import get_settings

        settings = get_settings()
        assert settings.app_env == "test", (
            f"测试环境期望 APP_ENV=test，实际 {settings.app_env}。"
            "请确认 APP_ENV_FILE=.env.test 已正确加载。"
        )
