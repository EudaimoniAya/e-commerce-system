...
async def test_create_order_by_seller_returns_201(
    ...
    # 库存已扣减
    assert await get_product_stock(db_session, product_id) == 8
...
async def test_create_order_by_seller_cross_shop_product_returns_422(
    ...
    own_category_id, own_product_id = await arrange_purchasable_product(
        db_session,
        shop_id=shop_owner.shop_id,
        stock=10,
    ...
    other_category_id = await seed_category(
        db_session,
        name=unique_category_name("cross-shop"),
    )
    other_product_id = await seed_product(
        db_session,
