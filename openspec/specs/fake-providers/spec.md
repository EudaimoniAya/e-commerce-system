...
产品侧测试替身治理：禁止 `production` 以 mock LLM / embedding provider 启动（fail-fast 门禁），并约定 AI 域测试替身目录 `tests/ai/testkit/`。对齐「Mock → Fake」命名重构（`FakeLLMClient`、`FakeEmbedder`、`FakeSmsProvider`），确保 mock 取值只出现在非 production 环境。
