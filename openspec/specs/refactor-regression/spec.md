...
应用层纪律（app layer discipline）重构回归护栏：证明对 `app/catalog`、`app/ordering`、`app/support`、`app/user` 的内部 refactor 不改变既有 HTTP API 外部行为，并通过 `scripts/check_app_layer_discipline.py` lint 门禁在 `task ci` 中强制层纪律（私有名不跨模块、跨域 import 白名单、`get_current_*` 仅本域 router 消费）。
