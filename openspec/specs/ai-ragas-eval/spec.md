...
#### Scenario: ragas 不列为生产依赖

- **WHEN** 检查项目依赖声明（`pyproject.toml`）
- **THEN** `ragas` SHALL 仅出现在可选依赖组（如 uv `eval` group）
- **AND** SHALL NOT 列入生产 `dependencies`
- **AND** 默认 `task ci` 环境 SHALL NOT 把 `ragas` 当作必装包
