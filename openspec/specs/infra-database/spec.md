...
系统 SHALL 通过 `pydantic-settings` 从环境变量加载配置，至少包含 `DATABASE_URL`（asyncmy 连接串）与 `APP_ENV`。配置不得硬编码于业务代码。
...
系统 SHALL 提供共享的 SQLAlchemy `DeclarativeBase`（或等价 `Base`），供 infra 与后续业务域 ORM 模型继承。

#### Scenario: Base 可被模型模块导入

- **WHEN** infra 验证模型继承 `Base`
