...
domain request schema 格式边界单元测试约定：在 `tests/unit/{domain}/test_{domain}_schema.py` 用纯 Pydantic 构造验证非法输入（parametrize + `ValidationError`），与 integration 业务路径测试分工；合法边界由 integration 覆盖。
