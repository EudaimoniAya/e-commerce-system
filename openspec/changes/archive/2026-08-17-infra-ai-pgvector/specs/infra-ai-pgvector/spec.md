...
PostgreSQL + pgvector AI 读副本基础设施：必填 `AI_DATABASE_URL`、独立 `AiBase` 与第二 Alembic 入口、`Embedder` 抽象与维度校验、devbox 本地提供与 CI service container、dev/test 双库约定；为后续 RAG 索引与检索提供统一 PG 与 embedding 访问入口。
...
系统 SHALL 通过 devbox 提供本地 PostgreSQL（含 **pgvector**）；SHALL 提供 `task pg:up` 与 `task pg:down` 及就绪轮询（`pg_isready`），镜像 `task redis:up` 纪律，并确保 dev/test AI 库存在。
...
- **WHEN** 开发者执行 `devbox run -- task pg:down`
- **THEN** devbox PostgreSQL 服务 SHALL 停止（在 devbox 托管范围内）
