...
#### Scenario: 全部依赖均不可用时返回 not_ready

- **WHEN** MySQL、Redis 与 PostgreSQL 均不可连接且客户端请求 `GET /health/ready`
- **THEN** 响应状态码 SHALL 为 503
- **AND** 响应体 JSON 的 `checks.mysql`、`checks.redis` 与 `checks.postgresql` SHALL 均为 `unavailable`
