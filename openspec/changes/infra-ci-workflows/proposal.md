...
- **Test workflow 重命名与简化**：`.github/workflows/ci.yml` → `test.yaml`（name: `Run Tests`）；去掉 domain matrix 与 `workflow_dispatch` 的 `domain` 输入；单 job 全量 `task test`（与本地 `task ci` 一致）
...
- `infra-ci`：废止 domain matrix 与 workflow_dispatch domain 筛选；新增 paths-filter、单 job 全量 test、workflow 重命名 `test.yaml`、failure-alert
...
- **保留不变**：`Dockerfile`、`.dockerignore`、Taskfile 域测试子命令（`test:user` 等，本地加速用）；Allure artifact 改为单 job 上传
