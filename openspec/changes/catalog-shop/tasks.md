...
- [ ] 2.1 扩展 `tests/conftest.py`：shop helpers（`create_shop_payload`、`register_and_open_shop`）与 `shop_owner` fixture（integration）
- [ ] 2.2 按 `specs/catalog-shop/spec.md` 编写 `tests/catalog/test_create_shop.py`（201、重复开店 422、店名冲突 422、未认证 401）；**不编写** catalog 实现
...
- [ ] 5.1 运行 `task migrate` 后 `task ci` 确认本地全绿；手动 `curl` 注册 → 开店 → `/shops/me` → patch closed → 公开 GET
