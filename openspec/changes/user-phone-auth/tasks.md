...
- [ ] 3.2 migration 回填 admin seed `phone='13800000000'`
- [ ] 3.3 更新 `tests/support/db/user.py`：`seed_active_user` / `seed_inactive_user` 支持可选 **`phone`**

## 4. SMS OTP 服务（绿 · 基础设施）

- [ ] 4.1 实现 `app/user/sms.py`：`SmsOtpService`（send、GETDEL consume、限流、register/login 成功后 DEL `sms:verify_fail:{phone}`）
...
- [ ] 5.2 扩展 `app/user/service.py`（`register_via_sms`、`login_via_sms`、password login；login 失败 dict detail `INVALID_CREDENTIALS`；扩展 `_to_user_response`）
- [ ] 5.3 更新 `app/user/router.py`（`/auth/sms/send`、`/auth/sms/register`、`/auth/sms/login`、修改 `/auth/login`、移除 `/auth/register`、移除 `/auth/sms/verify`、`PATCH /users/me`）
