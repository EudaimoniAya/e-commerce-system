...
- [ ] 1.1 `pyproject.toml` 增加 `loguru`；确认或补充 `.env.test` 中 `APP_ENV=test`；更新 `.env.example` 文档
...
- [ ] 2.1 按 `specs/infra-api-errors/spec.md` 编写 `tests/infra/test_error_handlers.py`（ValidationError、HTTPException 混合 code 解析、500 泛化、request_id）；**不编写** handlers
